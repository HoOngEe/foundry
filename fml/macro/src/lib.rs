...
pub fn handle_callable(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let source_trait = match ast {
        ...
        item => {
            syn::Error::new_spanned(item, "#[fml_callable] must be with a trait").to_compile_error();
            panic!();
        }
    };

    let name = source_trait.ident.clone();
    let name_con = Ident::new(&format!("{}_Con", name), source_trait.ident.span());
    ...
    for method in methods.iter() {
        impl_item.items.push(syn::ImplItem::Method(syn::ImplItemMethod {
            attrs: Vec::new(),
